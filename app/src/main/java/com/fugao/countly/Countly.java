			...
			public void run() {
				while (true) {
					...
					try {
						...
						HttpPost httpPostJson=new HttpPost(new URI(serverURL_));
						httpPostJson.addHeader("charset", HTTP.UTF_8);
						httpPostJson.addHeader("ContentType", "application/json");
						String data1="{\"Items\":[{\"Context\":\""+data+"\"}]}";
						StringEntity entityJson = new StringEntity(
								data1, HTTP.UTF_8);
						entityJson.setContentType("application/json; charset=utf-8");
						entityJson.setContentEncoding(new BasicHeader(
								HTTP.CONTENT_TYPE, "application/json; charset=utf-8"));
						httpPostJson.setEntity(entityJson);
