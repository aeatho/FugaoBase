...
public class SlidingLayer extends FrameLayout {
    ...
    public SlidingLayer(Context context, AttributeSet attrs, int defStyle) {
        ...
        setStickTo(ta.getInt(R.styleable.SlidingLayer_sl_stickTo, STICK_TO_AUTO));

        // Sets the shadow drawable
        int shadowRes = ta.getResourceId(R.styleable.SlidingLayer_sl_shadowDrawable, -1);
        if (shadowRes != -1) {
            setShadowDrawable(shadowRes);
        }

        // Sets the shadow width
        setShadowWidth((int) ta.getDimension(R.styleable.SlidingLayer_sl_shadowWidth, 0));

        // Sets the ability to close the layer by tapping in any empty space
        closeOnTapEnabled = ta.getBoolean(R.styleable.SlidingLayer_sl_closeOnTapEnabled, true);
        // Sets the ability to open the layout by tapping on any of the exposed closed layer
        openOnTapEnabled = ta.getBoolean(R.styleable.SlidingLayer_sl_openOnTapEnabled, true);

        // How much of the view sticks out when closed
        setOffsetWidth(ta.getDimensionPixelOffset(R.styleable.SlidingLayer_sl_offsetWidth, 0));
    ...
    /**
     * Sets the listener to be invoked after a switch change {@link com.fugao.view.SlidingLayer.OnInteractListener}.
     * 
     * @param listener
     *            Listener to set
     */
